...
const createElements = () => {
    ...
    elements.p6 = document.createElement('span');
...
const configElements = (elements,info) => {
    ...
    elements.p4.textContent = `${info.Compania}`;
    elements.p5.textContent = 'Email:';
    elements.p6.textContent = `${info.Email}`;
    elements.p7.textContent = 'Tipo de usuario:';

    info.Email.length > 21 &&  info.Email.length < 24 ? elements.p6.style.marginLeft = '-7px' : null;
    info.Email.length > 24 ? elements.p6.style.marginLeft = '-10px' : null;
